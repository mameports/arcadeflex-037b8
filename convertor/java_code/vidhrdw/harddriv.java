...
public class harddriv
{
	...
	public static VhStartPtr harddriv_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		for (i = 0; i < 65536; i++)
			...
			{
				mask = 0;
				if ((i & 0x0003) != 0) mask |= MASK(0);
				if ((i & 0x000c) != 0) mask |= MASK(1);
				if ((i & 0x0030) != 0) mask |= MASK(2);
				if ((i & 0x00c0) != 0) mask |= MASK(3);
				*destmask++ = mask;
	
				mask = 0;
				if ((i & 0x0300) != 0) mask |= MASK(0);
				if ((i & 0x0c00) != 0) mask |= MASK(1);
				if ((i & 0x3000) != 0) mask |= MASK(2);
				if ((i & 0xc000) != 0) mask |= MASK(3);
				*destmask++ = mask;
			}
			else
			{
	...
	public static VhUpdatePtr harddriv_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
