...
public class ginganin
{
	...
	***************************************************************************/
	...
	#define SETSCROLL \
		...
		for (offs = 0 ; offs < (spriteram_size[0] >> 1) ; offs += 4)
		{
			int x,y,code;
		...
		if ((layers_ctrl1 & 1) != 0)	tilemap_draw(bitmap, bg_tilemap,  0,0);
		else					fillbitmap(bitmap,palette_transparent_pen,&Machine.visible_area);
	
		if ((layers_ctrl1 & 2) != 0)	tilemap_draw(bitmap, fg_tilemap,  0,0);
		if ((layers_ctrl1 & 8) != 0)	draw_sprites(bitmap);
		if ((layers_ctrl1 & 4) != 0)	tilemap_draw(bitmap, tx_tilemap, 0,0);
