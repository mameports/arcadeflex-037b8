...
public class toypop
{
	...
	extern UBytePtr m68000_sharedram;
	...
	***************************************************************************/
	...
	public static VhUpdatePtr toypop_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		...
		for (offs = 0;offs < spriteram_size[0];offs += 2) {
			...
			if ((spriteram_2.read(offs)) != 0xe9) {
				...
				x = 343 - (spriteram_2.read(offs+1)| ((spriteram_3.read(offs+1)& 1) << 8));
				y = spriteram_2.read(offs)- 9;
				if (flipscreen != 0) {
					flipx = !flipx;
					flipy = !flipy;
				}
	...
	public static VhUpdatePtr liblrabl_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		register int offs, x, y;
