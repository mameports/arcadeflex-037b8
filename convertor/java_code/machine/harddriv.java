...
public class harddriv
{
	...
	*/
	...
	public static timer_callback duart_callback = new timer_callback() { public void handler(int param) 
	{
	...
	READ16_HANDLER( racedriv_asic65_r )
	{
		...
		if (!asic65_log) asic65_log = fopen("asic65.log", "w");
		if (asic65_log != 0) fprintf(asic65_log, " (R=%04X)", asic65_result[asic65_result_index]);
	...
	static READ16_HANDLER( asic61_r )
	{
		...
		else
		{
			...
			if (asic61_log != 0) fprintf(asic61_log, "Unexpected ASIC61 read from %06X\n", offset);
		}
		return 0;
	}
	...
	//			if (asic61_log != 0) fprintf(asic61_log, "%06X:mem R @ %08X=%04X\n", cpu_getpreviouspc(), asic61_addr, asic61_r(asic61_addr));
			...
			default:
	//			if (asic61_log != 0) fprintf(asic61_log, "%06X:R@%02X\n", cpu_getpreviouspc(), offset);
				break;
		}
		return 0;
	}
